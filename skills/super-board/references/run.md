...
| Rate-limit guard | Before each tick, call `platform_rate_remaining graphql` / `platform_rate_guard`. If GraphQL remaining < 200, sleep until reset. Prevents the runner from dying mid-loop when the user has burned quota in another tool. |
