<!-- GENERATED FILE — edit skills/super-board/references/platforms.md, then run scripts/generate-supersaiyan-references.sh to regenerate. Do not hand-edit. -->
...
`gitlab`). It is **orthogonal to `worker_backend`**.

GitLab is supported for **onboard** and **run via the bash dispatcher**
(`worker_backend: claude-p` / `codex-exec` / `cursor-agent`, string or per-lane
object). **lint, status, stop, the workflow backend, and super-review remain
GitHub-only.** `run-workflow.md` is still `gh`-literal for rate guard,
claim/release, and the lane allowlist. `workflow` staying Claude-Code-only is a
separate **backend** fact (see `references/backends.md`).
...
- Claim: GET assignees first; refuse if any username ≠ bot; `glab issue update --assignee <bot>` (never `assignee_ids=1,2`). Release: `assignee_ids=0` (empty `assignee_ids[]=` is HTTP 400). Open Judgment Call 3: release evicts co-assignees.
