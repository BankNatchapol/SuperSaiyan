...
# Scenario 9 — Ready card waits while its dependency card is not Done
DEPENDENT_ITEMS='{"items":[
  {"status":"Ready","content":{"type":"Issue","repository":"owner/repo","number":1,"title":"First","body":""}},
  {"status":"Ready","content":{"type":"Issue","repository":"owner/repo","number":2,"title":"Second","body":"## Notes\n- Depends on: #1"}}
]}'
OUT9=$("$PLAN" --config fixtures/wave-config.json --items <(echo "$DEPENDENT_ITEMS"))
echo "$OUT9" | jq -e '[.cards[].number] == [1]' >/dev/null || fail "open dependency should hold #2, got: $OUT9"

# Scenario 10 — dependent Ready card becomes eligible after prerequisite reaches Done
DEPENDENCY_DONE=$(echo "$DEPENDENT_ITEMS" | jq '(.items[] | select(.content.number == 1) | .status) = "Done"')
OUT10=$("$PLAN" --config fixtures/wave-config.json --items <(echo "$DEPENDENCY_DONE"))
echo "$OUT10" | jq -e '[.cards[].number] == [2]' >/dev/null || fail "Done dependency should release #2, got: $OUT10"

echo "PASS: test-wave-plan.sh (10 scenarios)"
